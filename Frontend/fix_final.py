...
start_tab = content.find('<!-- CONTINGENCY PLAN UI -->')
end_tab = content.find('<div class="comment-box mb-6" v-if="activityTab === \'comments\'">')
if start_tab != -1 and end_tab != -1:
    print("Found tab bounds")
else:
    print("Tab bounds not found")
    sys.exit(1)

new_tab_content = """<!-- CONTINGENCY PLAN UI -->
...
content = content[:start_tab] + new_tab_content + content[end_tab:]

# 2. Replace the Modal Dialogs at the end
start_modal = content.find('<!-- CONTINGENCY DIALOGS -->')
end_modal = content.find('</template>')
if start_modal != -1 and end_modal != -1:
    print("Found modal bounds")
else:
    print("Modal bounds not found")
    sys.exit(1)

new_modal_content = """<!-- CONTINGENCY DIALOGS -->
<el-dialog v-model="showContingencyForm" :title="editingContingencyPlanId ? 'Cập nhật Kế hoạch dự phòng' : 'Thêm Kế hoạch dự phòng mới'" width="600px" class="theme-dialog" :teleported="false">
   ...
   <div class="cm-form-group mt-3">
      <label class="cm-label">Ghi chú</label>
...
content = content[:start_modal] + new_modal_content + content[end_modal:]

# 3. Replace the JS logic
start_js = content.find('// Contingency Plan logic')
end_js = content.find('// Comments logic')
if start_js != -1 and end_js != -1:
    print("Found js bounds")
else:
    print("JS bounds not found")
    sys.exit(1)

new_js_content = """// Contingency Plan logic
...
"""
content = content[:start_js] + new_js_content + content[end_js:]

# Write back
with open('src/components/TaskDetailModal.vue', 'w', encoding='utf-8') as f:
    f.write(content)

print("Done restoring and fixing Contingency Plan UI completely!")
