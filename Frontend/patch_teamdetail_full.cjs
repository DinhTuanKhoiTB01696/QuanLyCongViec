...
// 1. Imports
if (!content.includes('useGoalStore')) {
...
// 2. Store initializations
if (!content.includes('goalStore = useGoalStore')) {
    content = content.replace(
...
// 3. Define siteGoals and siteProjects
if (!content.includes('const siteGoals')) {
    const goals_insertion = `const route = useRoute()`;
    content = content.replace(goals_insertion, goals_insertion + `

const siteGoals = computed(() => goalStore.goals || [])
const siteProjects = computed(() => homeProjectStore.projects || [])

const linkGoal = async (g) => {
  // Logic to link goal
  isGoalDropdownOpen.value = false
}

const linkProject = async (p) => {
  // Logic to link project
  isProjectDropdownOpen.value = false
}`);
}

// 4. Hierarchy layout (Grid & 'x' button)
const old_children = `          <div class="tree-level children-level" style="display: flex; flex-direction: column; align-items: center; position: relative; width: 100%;">
...
const new_children = `          <div class="tree-level children-level" style="display: flex; flex-direction: column; align-items: center; position: relative; width: 100%;">
...
// 5. Add removeChildTeam script logic
const old_addchild = `const addChildTeam = async (t) => {
...
const new_addchild = `const addChildTeam = async (t) => {
  if (!teamStore.hierarchy) teamStore.hierarchy = { parent: null, children: [] }
  if (!teamStore.hierarchy.children) teamStore.hierarchy.children = []
  if (!teamStore.hierarchy.children.find(c => c.id === t.id)) {
    teamStore.hierarchy.children.push(t)
...
// 6. Fix Goal dropdown position
const old_goal_text = `              <div style="flex: 1;">
...
const new_goal_text = `              <div style="flex: 1; position: relative;">
                 <h4 style="font-size: 14px; color: #172B4D; margin-bottom: 8px;">Xem các mục tiêu mà đội ngũ của bạn đang hướng tới</h4>
                 <p style="font-size: 13px; color: #6B778C; margin-bottom: 16px; line-height: 1.5;">Mục tiêu giúp đội ngũ của bạn kết nối công việc với những kết quả mà họ đóng góp, đồng thời cung cấp một nơi duy nhất để chia sẻ tiến độ thực hiện mục tiêu. Tạo mục tiêu để mọi người hiểu rõ định hướng và ưu tiên của nhóm.</p>
                 <button class="secondary-btn" @click.stop="isGoalDropdownOpen = !isGoalDropdownOpen">Thêm mục tiêu</button>
...
// 7. Fix Project tab dropdown
const old_proj_text = `              <div style="position: relative;">
...
const new_proj_text = `              <div style="position: relative;">
                 <div style="width: 80px; height: 80px; background-color: #EBECF0; border-radius: 8px; display: flex; align-items: center; justify-content: center; transform: rotate(-5deg);">
                    <i class="fa-solid fa-rocket" style="font-size: 32px; color: #172B4D;"></i>
                 </div>
...
// 8. Add project refs if missing
if (!content.includes('const isProjectDropdownOpen')) {
    const old_vars = `const isGoalDropdownOpen = ref(false)\nconst goalSearch = ref('')`;
    const new_vars = `const isGoalDropdownOpen = ref(false)\nconst goalSearch = ref('')\nconst isProjectDropdownOpen = ref(false)\nconst projectSearch = ref('')`;
    content = content.replace(old_vars, new_vars);
}

// 9. Fix tab pane
const old_projects_tab = `<div v-if="currentTab === 'projects'" class="tab-pane">`;
const new_projects_tab = `<div v-if="currentTab === 'projects'" class="tab-pane" @click="isProjectDropdownOpen = false">`;
content = content.replace(old_projects_tab, new_projects_tab);

fs.writeFileSync(path, content, 'utf8');
console.log("Full restoration and patch applied to TeamDetail.vue");
