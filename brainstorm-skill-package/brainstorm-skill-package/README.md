...
| L1 preview dùng bảng `# | path | action` | Thiếu rule `approval-gate.md` hoặc `ba-conventions.md` Mục 5 | Copy lại rule |
| Open Questions không được loop resolve sau Write | Thiếu rule `resolve-oqs.md` | Copy lại rule |

## 6. Attribution

Tác giả: BA Vault Template v2.x — `@hoangpm` (Hoang Phan).
